#!/usr/bin/env python3
"""Aggregate multi-run Zerodha bench reports into a summary table.

Parses per-run logs that contain (from engine.bench_zerodha)::
...
def main(argv: list[str] | None = None) -> int:
    ...
    title = (
        argv[4]
        if len(argv) > 4
        else "# Zerodha xN latest summary (pythoncoreengine harness)"
    )

    # Prefer the harness names used by run_bench_x*.sh / Makefile.
    candidates = list(out.glob("zerodha_run*.txt")) + list(
        out.glob("zerodha_nocomply_run*.txt")
    )
    # Also accept legacy zerodha_py_run*.txt if no modern names present.
    if not candidates:
        candidates = list(out.glob("zerodha_py_run*.txt"))
    paths = sorted(
        candidates,
