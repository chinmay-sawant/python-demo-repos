#!/usr/bin/env bash
# Zerodha gold standard (non-compliant): BENCH_ITERATIONS × 10 sequential runs.
# Same 80/15/5 workload with PDF/A, tagging, signing, and font embedding disabled.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT="${BENCH_OUT_DIR:-$REPO_ROOT/baselines/zerodha_bench_x10_nocomply}"
mkdir -p "$OUT"
...
STATS="${BENCH_STATS_PATH:-$REPO_ROOT/baselines/zerodha_bench_x10_nocomply_stats_latest.txt}"
...
lines = [
    "# Zerodha x10 non-compliant latest summary (gocorepdfengine harness)",
    "",
    f"Runs: {len(runs)}",
]
if runs:
