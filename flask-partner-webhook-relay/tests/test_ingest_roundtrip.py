...
class TestIngestRoundTrip:
    @pytest.mark.parametrize(
        "body",
        [
            b'{"event_type": "nested", "payload": {"nested": {"list": [1, 2, {"k": "v"}]}}, '
            b'"idempotency_key": "rt-1"}',
            b'{"event_type": "list.payload", "payload": [1, "two", {"three": 3}], '
            b'"idempotency_key": "rt-2"}',
            b'{"event_type": "scalar.int", "payload": 42, "idempotency_key": "rt-3"}',
            b'{"event_type": "scalar.str", "payload": "hello", "idempotency_key": "rt-4"}',
            b'{"event_type": "unicode", "payload": {"emoji": "\\ud83d\\ude80"}, '
            b'"idempotency_key": "rt-5"}',
    ...
    def test_ingest_parses_once_and_stores_raw(self, client, app):
        body = (
            b'{"event_type": "order.created", "payload": {"order_id": "x"}, '
            b'"idempotency_key": "rt-parse"}'
        )
        resp = client.post(
            "/api/v1/webhooks",
            data=body,
